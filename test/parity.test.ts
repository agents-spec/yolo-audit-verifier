...
import { recomputeAndAssess, type ProofBundle } from "../verify-client.ts";
...
type ParityFixture = {
  ...
  onChainRoot: string | null; onChainReachable: boolean; bundle: ProofBundle;
};
...
for (const fx of FIXTURES) {
  test(`parity (TS): ${fx.name}`, async () => {
    const view = await recomputeAndAssess(fx.bundle, fx.onChainRoot, fx.onChainReachable);
